...
type GreetServiceClient interface {
	...
	SayHelloClientrStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloClientrStreamingClient, error)
...
func (c *greetServiceClient) SayHelloClientrStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloClientrStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[1], "/greet_service.GreetService/SayHelloClientrStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayHelloClientrStreamingClient{stream}
	return x, nil
}

type GreetService_SayHelloClientrStreamingClient interface {
	Send(*HelloRequest) error
	CloseAndRecv() (*MessagesList, error)
	grpc.ClientStream
}

type greetServiceSayHelloClientrStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayHelloClientrStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayHelloClientrStreamingClient) CloseAndRecv() (*MessagesList, error) {
	if err := x.ClientStream.CloseSend(); err != nil {
		return nil, err
	}
	m := new(MessagesList)
...
type GreetServiceServer interface {
	...
	SayHelloClientrStreaming(GreetService_SayHelloClientrStreamingServer) error
...
func (UnimplementedGreetServiceServer) SayHelloClientrStreaming(GreetService_SayHelloClientrStreamingServer) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloClientrStreaming not implemented")
}
...
func _GreetService_SayHelloClientrStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayHelloClientrStreaming(&greetServiceSayHelloClientrStreamingServer{stream})
}

type GreetService_SayHelloClientrStreamingServer interface {
	SendAndClose(*MessagesList) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayHelloClientrStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayHelloClientrStreamingServer) SendAndClose(m *MessagesList) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayHelloClientrStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayHelloClientrStreaming",
			Handler:       _GreetService_SayHelloClientrStreaming_Handler,
			ClientStreams: true,
		},
		{
